...
import com.yangnk.hfn.common.adapter.HfnMessageListenerAdapter;
...
public class RabbitMQConfig {
    ...
    public SimpleMessageListenerContainer listenerContainer(@Qualifier("hfnMessageListenerAdapter") HfnMessageListenerAdapter hfnMessageListenerAdapter) throws Exception {
        String queueName = env.getProperty("mq.queue.hfn").trim();
    ...
    @Bean
    Queue queue() {
        String name = env.getProperty("mq.queue.hfn").trim();
        // 是否持久化
    ...
    TopicExchange exchange() {
        String name = env.getProperty("mq.exchange.hfn").trim();
        // 是否持久化
    ...
    @Bean
    Binding binding() {
        String routekey = env.getProperty("mq.routekey.hfn").trim();
